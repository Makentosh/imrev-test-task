...
import { logoutUser } from '../../api/userRequests.ts';
import { useAuthStore } from '../../store/authStore.ts';
import useAuth from '../../hooks/useAuth.ts';
import { ErrorResponseResult } from '../../types.ts';
...
const Navbar = () => {
  ...
  const handleLogout = () => {
    logoutUser()
        ...
        .catch((error: ErrorResponseResult) => {
          alert(error?.message);
          hardLogout();
        });
  };
                  ...
                  { navigation.map((item) => (
                      <a
                          ...
                          className={ classNames(
                              item.current
                                  ? 'bg-gray-900 text-white content-center rounded-2xl px-3 py-2'
          ...
          <div className="space-y-1 px-2 pb-3 pt-2">
            { navigation.map((item) => (
                <DisclosureButton
                    ...
                    aria-current={ item.current ? 'page' : undefined }
                    className={ classNames(
                        item.current ? 'bg-gray-900 text-white content-center' : 'text-gray-300 hover:bg-gray-700 hover:text-white',
